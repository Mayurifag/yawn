...
// TestSaveRawMessageLog tests that the raw message log is saved to the correct location.
func TestSaveRawMessageLog(t *testing.T) {
	// Create a temporary directory to simulate user config
	tempUserDir := t.TempDir()
	...
	// Sample content to log
	sampleContent := "This is a test message from Gemini API"

	// Call the function to test
	SaveRawMessageLog(sampleContent)

	// Check if the log file was created
	expectedLogPath := filepath.Join(tempUserDir, "latest_message.log")
	content, err := os.ReadFile(expectedLogPath)
	require.NoError(t, err, "Failed to read created log file")

	// Verify content
	assert.Equal(t, sampleContent, string(content), "Log file content doesn't match expected content")

	// Test overwrite behavior
	newContent := "Updated content for overwrite test"
	SaveRawMessageLog(newContent)

	// Verify content was overwritten
	updatedContent, err := os.ReadFile(expectedLogPath)
	require.NoError(t, err, "Failed to read updated log file")
	assert.Equal(t, newContent, string(updatedContent), "Log file wasn't properly overwritten")
}
